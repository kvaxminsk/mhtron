...
// если большое изображение отсутствует
// мочим все маленькие картинки, которые могли остаться
if(!getFileFormat($_SERVER['DOCUMENT_ROOT'].$path.$fname.".*"))
{
	$mas_dir = get_dir_list($_SERVER['DOCUMENT_ROOT'].$path);
	if(sizeof($mas_dir))
	{
		foreach($mas_dir as $dir)
		{
			$fe = getFileFormat($dir.$fname.".*");
			@unlink($dir.$fname.'.'.$fe);
		}
	}
}
